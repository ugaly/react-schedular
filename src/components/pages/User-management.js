...
const UserManagement = () => {
  ...
  const deleteUser = (id) => {
    AuthService.userDelete(id).then(r => {
      AuthService.loadByUsersRoleAndId(0, 1).then(response => {
        console.log(response.data)
        setData(response.data.results)
        setShowModalEdit(false)
  ...
  return (
    <div>
      ...
      <div className="flex  items-center h-10 intro-y">
        <h2 className="mr-5 text-lg font-medium truncate">User Management</h2>
      </div>

      <div className="relative mt-1 overflow-x-auto shadow-md bg-gray-100 sm:rounded-lg">
        <div className=" flex flex-col justify-between gap-8 md:flex-row md:items-center bg-white">
          <div></div>
      ...
      >
        ...
        <div className="py-1">
          ...
          <div className="flex w-full shrink-0 gap-2 md:w-max">
            ...
            <div className="w-full mt-1 pb-4 md:w-72">
              <input
                type="text"
                className="bg-dark text-dark mt-3 bb-3 rounded-md p-2 border border-black"
                placeholder="Search here"
              />
            </div>
            <button onClick={() => setShowModal(true)} className="flex items-center mt-3 mb-3 gap-2 mr-4 pr-2 pl-2 border rounded-md " >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
